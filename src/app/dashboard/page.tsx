...
import { Database, TrendingUp, Clock, Wallet, ChevronLeft, ChevronRight, CheckCircle } from 'lucide-react';
...
const UserDashboard = () => {
  ...
  // Helper function to format ETH amounts to 2 decimal places
  ...
  useEffect(() => {
    if (isConnected && address && publicClient) {
      ...
      if (cachedData) {
        console.log(`✅ Cache hit! Found ${cachedData.bids.length} cached bids`);
        console.log(`📊 Cache range: ${cachedData.earliestFetchedBlock} to ${cachedData.latestBlock}`);
        
        // Apply deduplication to cached data as well (safety measure)
        const uniqueCachedBids = deduplicateBids(cachedData.bids);
        console.log(`🔧 After cache deduplication: ${uniqueCachedBids.length} unique bids`);
        ...
        const hasScannedFromGenesis = BigInt(cachedData.earliestFetchedBlock) === BigInt(0);
        setHasMoreData(!hasScannedFromGenesis && BigInt(cachedData.earliestFetchedBlock) > BigInt(0));
        
        console.log(`🔄 Cache loaded. Has more data: ${!hasScannedFromGenesis && BigInt(cachedData.earliestFetchedBlock) > BigInt(0)} (scanned from genesis: ${hasScannedFromGenesis})`);
      } else {
  ...
  const getCachedData = (): { bids: BidEvent[], latestBlock: string, earliestFetchedBlock: string } | null => {
    ...
    try {
      ...
      console.log(`⏰ Cache age: ${Math.round(cacheAge / 1000 / 60)} minutes (expires after ${CACHE_EXPIRY_TIME / 1000 / 60} minutes)`);
  ...
  const fetchInitialUserBids = async () => {
    ...
    try {
      ...
      console.log(`🎯 Initial scan range: ${startBlock} to ${latestBlock} (${INITIAL_BLOCKS_RANGE.toLocaleString()} blocks - approx 7 days)`);
      
      let fetchedBids = await fetchUserBidsInRange(startBlock, latestBlock);
      console.log(`✅ Initial scan completed. Found ${fetchedBids.length} user bids in recent blocks`);
      ...
      if (fetchedBids.length === 0 && startBlock > BigInt(0)) {
        console.log(`⚠️ No bids found in recent ${INITIAL_BLOCKS_RANGE.toLocaleString()} blocks. Starting comprehensive scan from block 0...`);
        console.log(`🔍 Comprehensive scan range: 0 to ${startBlock - BigInt(1)} (${startBlock} blocks)`);
        
        // Scan from block 0 to the start of our previous range
        const olderBids = await fetchUserBidsInRange(BigInt(0), startBlock - BigInt(1));
        console.log(`✅ Comprehensive scan completed. Found ${olderBids.length} user bids in older blocks`);
        ...
        console.log(`🔧 After deduplication: ${fetchedBids.length} unique bids`);
        ...
        console.log(`✅ Found bids in recent blocks. Can load ${startBlock > BigInt(0) ? 'older blocks' : 'no more blocks'}`);
      ...
      if (fetchedBids.length > 0) {
        toast.success(`Successfully found ${fetchedBids.length} bid${fetchedBids.length > 1 ? 's' : ''}!`);
      } else {
      ...
      if (didComprehensiveScan) {
        console.log(`🏁 COMPLETE: Scanned all blocks (0 to ${latestBlock}) - found ${fetchedBids.length} bids`);
      } else if (fetchedBids.length > 0 && startBlock > BigInt(0)) {
        console.log(`📋 PARTIAL: Scanned recent blocks (${startBlock} to ${latestBlock}) - found ${fetchedBids.length} bids. Load more available.`);
      } else {
        console.log(`📋 COMPLETE: Scanned available blocks - found ${fetchedBids.length} bids`);
      }
    } catch (err) {
  ...
  const deduplicateBids = (bids: BidEvent[]): BidEvent[] => {
    ...
    for (const bid of bids) {
      if (!seen.has(bid.transactionHash)) {
        ...
        uniqueBids.push(bid);
        console.log(`✓ Added unique bid: ${bid.transactionHash.slice(0, 10)}...`);
      } else {
        console.log(`⚠️ Skipped duplicate bid: ${bid.transactionHash.slice(0, 10)}...`);
      }
    }
  ...
  const fetchMoreUserBids = async () => {
    ...
    console.log(`📚 Loading more bids. Current earliest block: ${earliestFetchedBlock}`);
    ...
    try {
      ...
      console.log(`📊 Found ${olderBids.length} additional bids in remaining blocks`);
      
      // Merge and deduplicate all bids
      const allBids = [...userBids, ...olderBids];
      const uniqueBids = deduplicateBids(allBids);
      
      console.log(`🔧 After deduplication: ${uniqueBids.length} total unique bids`);
      ...
      // Show success toast
      toast.success(`Successfully loaded all historical data! Found ${uniqueBids.length} total bid${uniqueBids.length > 1 ? 's' : ''}!`);
    } catch (err) {
  ...
  const fetchUserBidsInRange = async (fromBlock: bigint, toBlock: bigint): Promise<BidEvent[]> => {
    ...
    console.log(`🔎 Fetching bids in range: ${fromBlock} to ${toBlock} (${toBlock - fromBlock + BigInt(1)} blocks)`);
    ...
    try {
      ...
      console.log(`📡 Querying InsertBid events from contract: ${config.contracts.cacheManager.address}`);
      ...
      console.log(`📊 Found ${logs.length} total InsertBid events in this range`);
      
      // No logs found, return empty array
      if (logs.length === 0) {
        console.log(`ℹ️ No InsertBid events found in blocks ${fromBlock} to ${toBlock}`);
        return [];
      }
      
      console.log(`🔍 Processing events in batches of ${BATCH_SIZE}. Connected wallet: ${address}`);
      
      // Process events in larger batches for better performance
      for (let i = 0; i < logs.length; i += BATCH_SIZE) {
        const batch = logs.slice(i, i + BATCH_SIZE);
        console.log(`⚙️ Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(logs.length / BATCH_SIZE)} (${batch.length} events)`);
        ...
        console.log(`📥 Fetched ${transactions.length} transactions for current batch`);
        
        // Filter logs for transactions sent by the connected wallet
        const userLogsIndices = transactions
          .map((tx, idx) => {
            const isUserTransaction = tx.from.toLowerCase() === address.toLowerCase();
            if (isUserTransaction) {
              console.log(`✅ Found user transaction: ${tx.hash} (from: ${tx.from})`);
            }
        ...
        console.log(`👤 Found ${userLogsIndices.length} user transactions in this batch`);
        
        if (userLogsIndices.length === 0) {
          console.log(`⏭️ No user transactions in this batch, skipping to next`);
          continue;
        }
        ...
        for (const log of userLogs) {
          ...
          console.log(`💰 Added user bid: ${formatEther(bidEvent.args.bid)} ETH for program ${formatAddress(bidEvent.args.program)} (tx: ${bidEvent.transactionHash.slice(0, 10)}...)`);
        }
      ...
      console.log(`📈 Statistics: ${stats.totalEvents} total events, ${stats.userEvents} user events, ${stats.processedBlocks} blocks processed`);
      ...
      console.error(`❌ Error fetching bids in range ${fromBlock} to ${toBlock}:`, error);
      throw error;
    }
  };
  ...
  const clearCache = () => {
    if (!address) return;
    console.log(`🗑️ Clearing cache and refreshing data for address: ${address}`);
    ...
    console.log(`🔄 Starting fresh scan - will check latest 2.5M blocks first, then older blocks if needed`);
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <div className="text-2xl font-bold">
                    {formatEthAmount(getTotalBidAmount())} ETH
                  </div>
                </div>
                          ...
                          <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            <td className="py-3 px-4">
                              <div className="flex items-center gap-2">
                                ...
                                <button 
                                  onClick={() => {
                                    navigator.clipboard.writeText(bid.args.program);
